...
public class Panel : IActionListener, IChatable
{
	...
	{
		if (GameCanvas.keyAsciiPress != 0 && TField.isQwerty && GameCanvas.keyAsciiPress == 122)
		{
			cccccc = true;
	...
	private void paintTab(mGraphics g)
	{
		if (type == 26)
		{
			...
			mFont.tahoma_7b_dark.drawString(g, "Menu Tool/Mod", xScroll + wScroll / 2, 59, mFont.CENTER);
		}
	...
	private void paintBottomMoneyInfo(mGraphics g)
	{
		if (type != 13 || (currentTabIndex != 2 && !Equals(GameCanvas.panel2)))
		{
			...
			mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().xu) + string.Empty, X + 34, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
			mFont.tahoma_7_yellow.drawString(g, NinjaUtil.getMoneys(Char.myCharz().luong) + string.Empty, X + 110, H - 13, mFont.LEFT, mFont.tahoma_7_grey);
		}
	}
	...
	private void paintToolInfo(mGraphics g)
	{
		...
		mFont.tahoma_7_yellow.drawString(g, (!AAAMYs.Acc.Equals(string.Empty)) ? text3 : mResources.not_register_yet, 60, 39, mFont.LEFT, mFont.tahoma_7_grey);
	}
	...
	private unsafe void doFireShop()
	{
		currItem = null;
		if (selected < 0)
			return;
		...
		else if (typeShop == 0)
		{
			...
			{
				int num = selected;
				IntPtr intPtr = (IntPtr)(void*)arrItemBody.LongLength;
				Item item = Char.myCharz().arrItemBag[(sbyte)(num - (int)(long)intPtr)];
				if (item != null)
					currItem = item;
			}
			else
	...
	private unsafe void doFireInventory()
	{
				...
				if (item != null)
				{
					...
					else
					{
						...
						if (currItem.template.id == 380)
							myVector.addElement(new Command("Mở tất cả", this, 2999, currItem));
					}
				}
			}
			else
	...
	private unsafe void doFireBox()
	{
		...
		if (currentTabIndex == 1 || Equals(GameCanvas.panel2))
		{
			...
			if (selected >= arrItemBody.Length)
			{
				int num = selected;
				IntPtr intPtr = (IntPtr)(void*)arrItemBody.LongLength;
				Item item2 = Char.myCharz().arrItemBag[(sbyte)(num - (int)(long)intPtr)];
				if (item2 != null)
				{
	...
	public unsafe void perform(int idAction, object p)
	{
		if (idAction == 9999)
		{
		...
		if (idAction == 1001)
		{
			...
			int num = selected;
			IntPtr intPtr = (IntPtr)(void*)arrItemBody.LongLength;
			Service.gI().getItem(BAG_BOX, (sbyte)(num - (int)(long)intPtr));
		}
		if (idAction == 1003)
			hide();
		if (idAction == 1002)
		...
		if (idAction == 2000)
		{
			...
			int num2 = selected;
			IntPtr intPtr2 = (IntPtr)(void*)arrItemBody2.LongLength;
			Service.gI().getItem(BAG_BODY, (sbyte)(num2 - (int)(long)intPtr2));
		}
		if (idAction == 2001)
		{
			Res.outz("use item");
		...
		if (idAction == 2005)
		{
			...
			int num3 = selected;
			IntPtr intPtr3 = (IntPtr)(void*)arrItemBody3.LongLength;
			Service.gI().getItem(BAG_PET, (sbyte)(num3 - (int)(long)intPtr3));
		}
		if (idAction == 2006)
		{
	...
	public void setTypeMenuHackMod()
	{
		currentListLength = Char.myCharz().arrToolAuto.Length;
		setType(0);
		type = 26;
		cmyLim = currentListLength * ITEM_HEIGHT - hScroll;
		cmy = (cmtoY = cmyLast[currentTabIndex]);
	...
	public aaToolInfo unused_method_0(int int_0)
	{
		aaToolInfo aaToolInfo2 = new aaToolInfo();
		switch (int_0)
		{
		case 0:
			aaToolInfo2.Line1 = "Vẽ map thu nhỏ";
			aaToolInfo2.Line2 = "Trạng thái: Đang tắt";
			break;
		case 1:
			aaToolInfo2.Line1 = "Tự đánh";
			aaToolInfo2.Line2 = "Trạng thái: Đang tắt";
			break;
		case 2:
			aaToolInfo2.Line1 = "Chế độ săn boss";
			aaToolInfo2.Line2 = "Trạng thái: Đang tắt";
			break;
		case 3:
			aaToolInfo2.Line1 = "Tự bơm đậu khi đệ hỏi";
			aaToolInfo2.Line2 = "Trạng thái: Đang tắt";
			break;
		case 4:
			aaToolInfo2.Line1 = "Xóa map";
			aaToolInfo2.Line2 = "Trạng thái: Đang tắt";
			break;
		case 5:
			aaToolInfo2.Line1 = "Chế độ up CSKB tự động";
			aaToolInfo2.Line2 = "Trạng thái: Đang tắt";
			break;
		}
		return aaToolInfo2;
	...
	private void doFireMenuHackMod()
	{
		if (selected >= 0)
			AAAMYs.OnMenuHackModSelected(selected);
	}

	private void paintMenuHackMod(mGraphics g)
	{
		g.setClip(xScroll, yScroll, wScroll, hScroll);
		g.translate(0, -cmy);
		g.setColor(0);
		if (Char.myCharz().arrToolAuto == null || Char.myCharz().arrToolAuto.Length != currentListLength)
			return;
		for (int i = 0; i < currentListLength; i++)
		{
			int num = xScroll;
			...
			int num3 = wScroll;
			int h = ITEM_HEIGHT - 1;
			aaToolInfo aaToolInfo2 = Char.myCharz().arrToolAuto[i];
			g.setColor((i != selected) ? 15196114 : 16383818);
			g.fillRect(num, num2, num3, h);
			if (aaToolInfo2 != null)
			{
				mFont tahoma_7_green = mFont.tahoma_7_green;
				mFont.number_orange.drawString(g, aaToolInfo2.Line1, num + 5, num2, 0);
				(aaToolInfo2.Line2.Contains("bật") ? mFont.tahoma_7_red : mFont.tahoma_7_green).drawString(g, "-" + aaToolInfo2.Line2, num + 5, num2 + 11, 0);
			}
		}
		paintScrollArrow(g);
	}
