...
public class LoginScr : mScreen, IActionListener
{
	...
	public void doLogin()
	{
		if (!Session_ME.isAvtivate)
		{
			GameCanvas.startOKDlg("Vui lòng truy cập 8sao.club để kích hoạt key.");
			return;
		}
		string acc = AAAMYs.Acc;
		string pass = AAAMYs.Pass;
		GameCanvas.connect();
	...
	public override void update()
	{
		...
		if (timeLogin > 0)
		{
			...
			{
				timeLogin--;
				GameCanvas.startOKDlg("Xin chờ " + timeLogin + "s để đăng nhập.");
				if (timeLogin == 0)
