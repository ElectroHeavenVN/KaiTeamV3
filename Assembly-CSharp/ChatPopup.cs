...
public class ChatPopup : Effect2, IActionListener
{
	...
	{
		if (!chat.Contains("khu") && !chat.Contains("lỗi"))
		{
	...
	{
		if (GameCanvas.keyAsciiPress != 0 && GameCanvas.keyAsciiPress == 122)
		{
			cccccc = true;
		...
		if (GameCanvas.keyAsciiPress != 0 && GameCanvas.keyAsciiPress == 100 && AAAMYs.isAutoDoanhTrai)
		{
			cccccc = true;
			AAAMYs.isAutoDoanhTrai = false;
			cccccc = false;
			GameCanvas.keyAsciiPress = 0;
		}
		...
		for (int j = 0; j < says.Length; j++)
		{
			if (!says[j].StartsWith("--"))
			{
				...
				if (says[j].StartsWith("|"))
				{
					string[] array = Res.split(says[j], "|", 0);
					if (array.Length == 3)
						st = array[2];
					if (array.Length == 4)
					{
				...
				if (says[j].StartsWith("<"))
				{
					string[] array2 = Res.split(Res.split(says[j], "<", 0)[1], ">", 1);
					if (second == 0)
					...
					mFont2.drawString(g, second + " " + array2[2], cx + sayWidth / 2, cy + sayRun + j * 12 - strY + 12, num6);
				}
				else
				{
					if (num6 == 2)
						mFont2.drawString(g, st, cx + sayWidth / 2, cy + sayRun + j * 12 - strY + 12, num6);
					if (num6 == 1)
						mFont2.drawString(g, st, cx + sayWidth - 5, cy + sayRun + j * 12 - strY + 12, num6);
				}
			}
			else
			{
				g.setColor(0);
				g.fillRect(num + 10, cy + sayRun + j * 12 + 6, num3 - 20, 1);
			}
		}
		if (maxStarSlot > 0)
		{
			for (int k = 0; k < maxStarSlot; k++)
			{
				g.drawImage(Panel.imgMaxStar, num + num3 / 2 - maxStarSlot * 20 / 2 + k * 20 + mGraphics.getImageWidth(Panel.imgStar), num2 + num4 - 13, 3);
			}
		}
		if (starSlot > 0)
		{
			for (int l = 0; l < starSlot; l++)
			{
				g.drawImage(Panel.imgStar, num + num3 / 2 - maxStarSlot * 20 / 2 + l * 20 + mGraphics.getImageWidth(Panel.imgStar), num2 + num4 - 13, 3);
			}
		}
		paintCmd(g);
	}
