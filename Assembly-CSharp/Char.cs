...
public class Char : IMapObject
{
	...
	{
		if (info.Contains("cho con đậu thần") && !myCharz().doUsePotion())
	...
	private void paintCharName_HP_MP_Overhead(mGraphics g)
	{
		...
		if ((paintName || flag2 || flag3) && !flag)
		{
			if (mSystem.clientType == 1)
				mFont2.drawString(g, cName + " " + cHP * 100 / cHPFull + "%", cx, cy - num, mFont.CENTER, mFont.tahoma_7_greySmall);
			else
				mFont2.drawString(g, cName + " " + cHP * 100 / cHPFull + "%", cx, cy - num, mFont.CENTER);
		...
		if (flag)
		{
			...
			{
				mFont2.drawString(g, cName + " " + cHP * 100 / cHPFull + "%", cx - 10, cy - num + 3, mFont.LEFT, mFont.tahoma_7_grey);
