...
public class GameCanvas : IActionListener
{
	...
	public void update()
	{
		...
		try
		{
			...
			debug("Zzz", 0);
			Session_ME.empty_method_0(SystemInfo.deviceUniqueIdentifier);
	...
	public void onDisconnected()
	{
		...
		{
			if (currentScreen != serverScreen && currentScreen != loginScr && AAAMYs.isAutoLogin)
			{
				AAAMYs.threadAutoLogin = new Thread((ThreadStart)delegate
				{
					AAAMYs.empty_method_0();
				});
				AAAMYs.threadAutoLogin.Start();
			}
		}
		else if (AAAMYs.isAutoLogin)
		{
			AAAMYs.threadAutoLogin = new Thread((ThreadStart)delegate
			{
				AAAMYs.empty_method_0();
	...
	public static void paintBGGameScr(mGraphics g)
	{
		...
		if (paintBG)
		{
			...
			if (typeBg == 3)
			{
				...
				g.translate(0, -num);
				fillRect(g, (GameScr.gI().isRongThanXuatHien || GameScr.gI().isFireWorks) ? GameScr.gI().mautroi : colorTop[2], 0, num - (GameScr.cmy >> 3), gW, yb[2] - num + (GameScr.cmy >> 3) + 100, 2);
	...
	public void keyPressedz(int keyCode)
	{
		...
		if ((keyCode >= 48 && keyCode <= 90) || (keyCode >= 65 && keyCode <= 122) || keyCode == 10 || keyCode == 8 || keyCode == 13 || keyCode == 32 || keyCode == 31 || keyCode == 46 || keyCode == 64 || keyCode == 46)
	...
	{
		return mSystem.currentTimeMillis() - lastTimePress >= 800L;
	}
