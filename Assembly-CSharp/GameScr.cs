...
public class GameScr : mScreen, IChatable
{
				...
				if (Char.myCharz().mobFocus != null)
				{
					...
					if (Char.myCharz().mobFocus.status != 1 && Char.myCharz().mobFocus.status != 0 && Char.myCharz().myskill.template.type != 4)
					{
						...
						if (num <= Char.myCharz().myskill.dx)
							num6 = 0;
						if (GetYGround(Char.myCharz().cx + 50 * Char.myCharz().cdir) < Char.myCharz().cy)
							Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().mobFocus.getX() + num6, GetYGround(Char.myCharz().cx + 50 * Char.myCharz().cdir));
						else
				...
				if (Char.myCharz().charFocus != null)
				{
					...
					if (!Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus) && !Char.myCharz().isSelectingSkillBuffToPlayer())
					{
						...
						else
						{
							...
							if (GetYGround(Char.myCharz().cx + 50 * Char.myCharz().cdir) < Char.myCharz().cy)
								Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().charFocus.cx + num12, GetYGround(Char.myCharz().cx + 50 * Char.myCharz().cdir));
							else
					...
					if (num10 <= Char.myCharz().myskill.dx)
						num15 = 0;
					if (GetYGround(Char.myCharz().cx + 50 * Char.myCharz().cdir) < Char.myCharz().cy)
						Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().charFocus.cx + num15, GetYGround(Char.myCharz().cx + 50 * Char.myCharz().cdir));
					else
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						AAAMYs.UpdateKey(GameCanvas.keyAsciiPress);
					}
	...
	private void autoPlay()
	{
		if (timeSkill > 0)
			timeSkill--;
		if (!canAutoPlay || Char.myCharz().isCharge)
			return;
		bool flag = false;
		...
		if (Char.myCharz().mobFocus != null && (Char.myCharz().mobFocus == null || !Char.myCharz().mobFocus.isMobMe))
		{
			...
			{
				if (Char.myCharz().mobFocus.checkIsBoss() && !AAAMYs.casieuquai)
					return;
				Char.myCharz().mobFocus = null;
		...
		{
			Char.myCharz().charFocus = null;
			if (!AAAMYs.autopickkhitansat && !AAAMYs.pickts2)
				Char.myCharz().itemFocus = null;
			else
			{
				if (AAAMYs.autopickkhitansat)
				{
					AAAMYs.PickItemWhenTanSat();
					if (AAAMYs.autopickplay)
					{
						AAAMYs.PickItemWhenTanSat();
						if (AAAMYs.autopickplay)
							return;
				...
				if (AAAMYs.pickts2)
				{
					AAAMYs.PickItemWhenTanSat2();
					if (AAAMYs.tsplay22222)
					{
						AAAMYs.PickItemWhenTanSat2();
						if (AAAMYs.tsplay22222)
							return;
		...
		if (GameCanvas.isTouch)
		{
			...
			{
				if (onScreenSkill[j] == null || onScreenSkill[j].paintCanNotUseSkill || onScreenSkill[j].template.id == 10 || onScreenSkill[j].template.id == 11 || onScreenSkill[j].template.id == 14 || onScreenSkill[j].template.id == 23 || onScreenSkill[j].template.id == 7 || Char.myCharz().skillInfoPaint() != null)
					continue;
				int num = ((onScreenSkill[j].template.manaUseType == 2) ? 1 : ((onScreenSkill[j].template.manaUseType == 1) ? (onScreenSkill[j].manaUse * Char.myCharz().cMPFull / 100) : onScreenSkill[j].manaUse));
				if (Char.myCharz().cMP >= num)
				{
					if (skill == null)
						skill = onScreenSkill[j];
					else if (skill.coolDown < onScreenSkill[j].coolDown)
					{
						skill = onScreenSkill[j];
					}
				}
		...
		for (int k = 0; k < keySkill.Length; k++)
		{
			if (keySkill[k] == null || keySkill[k].paintCanNotUseSkill || keySkill[k].template.id == 10 || keySkill[k].template.id == 11 || keySkill[k].template.id == 14 || keySkill[k].template.id == 23 || keySkill[k].template.id == 7 || Char.myCharz().skillInfoPaint() != null)
				continue;
			int num2 = ((keySkill[k].template.manaUseType == 2) ? 1 : ((keySkill[k].template.manaUseType == 1) ? (keySkill[k].manaUse * Char.myCharz().cMPFull / 100) : keySkill[k].manaUse));
			if (Char.myCharz().cMP >= num2)
			{
				if (skill == null)
					skill = keySkill[k];
				else if (skill.coolDown < keySkill[k].coolDown)
				{
					skill = keySkill[k];
				}
			}
	...
	public override void update()
	{
		...
		if (GameCanvas.gameTick % 100 == 0 && TileMap.mapID == 137)
			shock_scr = 30;
		if (isAutoPlay && GameCanvas.gameTick % AAAMYs.speedts == 0)
			autoPlay();
		updateXoSo();
	...
	public override void paint(mGraphics g)
	{
		...
		if (flag && !isRongThanXuatHien)
		{
			...
			if (TileMap.tileID != 28)
			{
				for (int num12 = 0; num12 < num11; num12++)
				{
					...
					int num14 = -20;
					if (num13 + mGraphics.getImageWidth(TileMap.imgLight) >= cmx && num13 <= cmx + GameCanvas.w && num14 + mGraphics.getImageHeight(TileMap.imgLight) >= cmy && num14 <= cmy + GameCanvas.h)
		...
		for (int num15 = 0; num15 < vCharInMap.size(); num15++)
		{
			...
			try
			{
				char6 = (Char)vCharInMap.elementAt(num15);
			}
			catch (Exception)
			{
			}
		...
		resetTranslate(g);
		int num16 = 100 + ((Char.vItemTime.size() != 0) ? (textTime.size() * 12) : 0);
		if (Char.myCharz().clan != null)
		{
			int num17 = 0;
			int num18 = 0;
			int num19 = (GameCanvas.h - 100 - 60) / 12;
			for (int num20 = 0; num20 < vCharInMap.size(); num20++)
			{
				Char char7 = (Char)vCharInMap.elementAt(num20);
				...
				if (char7.isOutX() && char7.cx < Char.myCharz().cx)
				{
					...
					if (num17 <= num21)
					{
						mFont.tahoma_7_green.drawString(g, char7.cName, 20, num16 - 12 + num17 * 12, mFont.LEFT, mFont.tahoma_7_grey);
						char7.paintHp(g, 10, num16 + num17 * 12 - 5);
						num17++;
					}
				}
				else if (char7.isOutX() && char7.cx > Char.myCharz().cx && num18 <= num19)
				{
					mFont.tahoma_7_green.drawString(g, char7.cName, GameCanvas.w - 25, num16 - 12 + num18 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
					char7.paintHp(g, GameCanvas.w - 15, num16 + num18 * 12 - 5);
					num18++;
				}
			}
		}
		...
		for (int num22 = 0; num22 < Char.vItemTime.size(); num22++)
		{
			((ItemTime)Char.vItemTime.elementAt(num22)).paint(g, cmdMenu.x + 32 + num22 * 24, 55);
		}
		for (int num23 = 0; num23 < textTime.size(); num23++)
		{
			((ItemTime)textTime.elementAt(num23)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 45 : 90) + num23 * 12);
		}
		paintXoSo(g);
		if (mResources.language == 1)
		{
			long num24 = mSystem.currentTimeMillis();
			long num25 = deltaTime;
			mFont.tahoma_7b_white.drawString(g, NinjaUtil.getDate2(num24 + num25), 10, GameCanvas.h - 65, 0, mFont.tahoma_7b_dark);
		}
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num3; i++)
				{
					if (Main.isPC)
					{
						...
						if (TField.isQwerty)
						{
							string[] obj = new string[8] { "1", "2", "3", "4", "5", "6", "7", null };
							array2 = obj;
							obj[7] = "8";
						}
						else
	...
	public void chatVip(string chatVip)
	{
		...
		{
			if (AAAMYs.listboss.Count > 4)
				AAAMYs.listboss.RemoveAt(0);
			AAAMYs.listboss.Add(chatVip.Replace(" vừa xuất hiện tại", " -=>") + "-" + DateTime.Now.ToString("HH:mm:ss"));
		}
